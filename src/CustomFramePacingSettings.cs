...
namespace MoonWorks
{
	...
	/// </summary>
	public enum CustomFramePacingMode
	{
		/// <summary>
	...
	/// </summary>
	public struct CustomFramePacingSettings
	{
		/// <summary>
		/// Specifies the frame pacing strategy of the Game's tick loop.
		/// </summary>
		public CustomFramePacingMode Mode { get; private set;}
		...
		/// </summary>
		public static CustomFramePacingSettings CreateLatencyOptimized(
			int timestepFPS,
			int maxUpdatesPerTick
		) {
			return new CustomFramePacingSettings(
				CustomFramePacingMode.LatencyOptimized,
				timestepFPS,
				timestepFPS,
				maxUpdatesPerTick
			);
		...
		/// </summary>
		public static CustomFramePacingSettings CreateCapped(
			int timestepFPS,
			int framerateCapFPS,
			int maxUpdatesPerTick
		) {
			return new CustomFramePacingSettings(
				CustomFramePacingMode.Capped,
				timestepFPS,
				framerateCapFPS,
				maxUpdatesPerTick
			);
		...
		/// </summary>
		public static CustomFramePacingSettings CreateUncapped(
			int timestepFPS,
			int maxUpdatesPerTick
		) {
			return new CustomFramePacingSettings(
				CustomFramePacingMode.Uncapped,
				timestepFPS,
				0,
				maxUpdatesPerTick
			);
		...
		private CustomFramePacingSettings(
			CustomFramePacingMode mode,
			int timestepFPS,
			int framerateCapFPS,
			int maxUpdatesPerTick
		) {
			...
			FramerateCapTimestep = mode != CustomFramePacingMode.Uncapped ? TimeSpan.FromTicks(TimeSpan.TicksPerSecond / framerateCapFPS) : TimeSpan.Zero;
		}
	}
}
