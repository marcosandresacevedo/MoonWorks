...
namespace MoonWorks
{
	...
	public abstract class CustomGame
	{
		...
		public CustomFramePacingSettings FramePacingSettings { get; private set; }
		...
		public unsafe CustomGame(
			AppInfo appInfo,
			WindowCreateInfo windowCreateInfo,
			CustomFramePacingSettings framePacingSettings,
		...
		/// </summary>
		public void SetFramePacingSettings(CustomFramePacingSettings settings)
		{
			/*
			 I don't want the AllowedFramesInFlight changing to 2 just
			 because I didn't use CustomFramePacingMode.LatencyOptimized.

			if (settings.Mode == CustomFramePacingMode.LatencyOptimized)
			{
		...
		private void Tick(bool processEvents)
		{
			...
			if (processEvents)
			{
				if (FramePacingSettings.Mode != CustomFramePacingMode.Uncapped)
				{
					// If we are in latency-optimized mode, we use the game timestep. Otherwise, we use the framerate cap.
					var capTimespan = FramePacingSettings.Mode == CustomFramePacingMode.Capped ? FramePacingSettings.FramerateCapTimestep : FramePacingSettings.Timestep;

					// We want to wait until the framerate cap,
					// but we don't want to oversleep. Requesting repeated 1ms sleeps and
					// seeing how long we actually slept for lets us estimate the worst case
					// sleep precision so we don't oversleep the next frame.
					while (AccumulatedDrawTime + WorstCaseSleepPrecision <  FramePacingSettings.Timestep)
					{
			...
			if (FramePacingSettings.Mode == CustomFramePacingMode.LatencyOptimized)
			{
			...
			var alpha = FramePacingSettings.Mode == CustomFramePacingMode.LatencyOptimized ?
				0 :
				(AccumulatedUpdateTime / FramePacingSettings.Timestep);
